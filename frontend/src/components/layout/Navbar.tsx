      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20 gap-4">
          ...
          <nav className="hidden md:flex items-center gap-8">
            <Link
              to="/"
              className="text-sm font-medium text-slate-700 dark:text-slate-200 hover:text-zinc-900 dark:text-zinc-100 dark:hover:text-zinc-400 transition-colors"
            >
              Home
            </Link>
            <Link
            ...
            {isAdmin && (
              <Link
                to="/admin"
                className="flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full bg-zinc-100 dark:bg-zinc-800/60 text-zinc-900 dark:text-zinc-100 border border-zinc-300/50 dark:border-zinc-700/50 hover:bg-zinc-200 dark:hover:bg-zinc-800/60 transition-colors"
              >
            ...
            <form onSubmit={handleSearchSubmit}>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-9 py-2 text-sm bg-slate-100 dark:bg-slate-800/80 border border-transparent focus:border-zinc-500 dark:focus:border-zinc-500 rounded-full focus:outline-none focus:ring-2 focus:ring-zinc-500/20 text-slate-900 dark:text-slate-100 placeholder-slate-400 transition-all"
                />
                {searchTerm && (
                  <button
                    type="button"
            ...
            {isSearchOpen && filteredProducts.length > 0 && (
              <div className="absolute left-0 right-0 mt-2 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden z-50 animate-fade-in">
                  ...
                  {filteredProducts.map((p) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-zinc-900 dark:text-zinc-100 font-semibold">
                ...
                <button
                  ...
                  className="w-full py-2.5 text-center text-xs font-semibold text-zinc-900 dark:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800/40 border-t border-slate-100 dark:border-slate-800"
                >
                ...
                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 p-2 z-50 animate-fade-in">
                  ...
                  {isAdmin && (
                    <Link
                      to="/admin"
                      onClick={() => setIsUserMenuOpen(false)}
                      className="flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-medium text-zinc-900 dark:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800/40 transition-colors"
                    >
            ...
            <nav className="flex flex-col gap-2">
              ...
              {isAdmin && (
                <Link
                  to="/admin"
                  onClick={() => setIsMobileNavOpen(false)}
                  className="px-3 py-2 rounded-lg text-sm font-semibold text-zinc-900 dark:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800/40"
                >
                  Admin Panel
                </Link>
              )}
